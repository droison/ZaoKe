...
public class LocListHttp implements Runnable {
	private Context context;
	private Handler mHandler;
	private String url;

	public LocListHttp(Context context, Handler mHandler) {
		this.context = context;
		this.mHandler = mHandler;
		this.url = AppConstant.HTTPURL.locList;
	}
	...
	public void run() {
		...
		case 200:
			try {
				...
				LocListBase llb = (LocListBase) JsonUtil.jsonToObject(json, LocListBase.class);
				mHandler.sendMessage(mHandler.obtainMessage(AppConstant.HANDLER_MESSAGE_NORMAL, llb));
			} catch (Exception e) {
